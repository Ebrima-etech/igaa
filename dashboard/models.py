...
class ChatMessage(models.Model):
    """Peer-to-peer chat messages between users (like WhatsApp)"""
    ...
    class Meta:
        ordering = ['-created_at']
        indexes = [
            models.Index(fields=['user', 'recipient', '-created_at']),
            models.Index(fields=['recipient', '-created_at']),
        ]

    def __str__(self):
        return f"{self.user.username} → {self.recipient.username}: {self.message[:50]}"
