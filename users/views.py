...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def register(self, request):
        try:
            logger.info(f"Register request method: {request.method}")
            logger.info(f"Register request content-type: {request.content_type}")
            logger.info(f"Register request data: {request.data}")
            logger.info(f"Register request POST: {request.POST}")
            ...
            logger.error(f"Exception in register: {type(e).__name__}: {str(e)}", exc_info=True)
            return Response(
                {'error': f'Error: {str(e)}'},
                status=status.HTTP_400_BAD_REQUEST
            )
